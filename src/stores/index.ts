...
const setActive = () => {
   ...
   if(theme === 'theme-red'){
      ...
      if(task){
         setActiveTask({min: task.time.current.min, sec: task.time.current.sec, count: task.count.current, name: task.name, id: task.id})
      }else{
         setActiveTask({min: 30, sec: 0, count:1, name: "", id: -1})
      }
   }else if(theme === 'theme-teal'){
      setActiveTask({min: 5, sec: 0, count: 1, name: "shortBreak", id: -1})
